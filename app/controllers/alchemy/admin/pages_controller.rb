module Alchemy
  module Admin
    class PagesController < Alchemy::Admin::BaseController
      ...
      Node = Struct.new(:left, :right, :parent, :depth)
      ...
      def visit(nodes, my_left, parent, depth, tree)
        nodes.each do |item|
          my_right = my_left + 1
          if item['children']
            my_right, tree = visit(item['children'], my_left+1, item['id'], depth+1, tree)
          end
          tree[item['id']] = Node.new(my_left, my_right, parent, depth)
          my_left = my_right + 1
        end
        [my_left, tree]
      end
      def create_tree(items, rootpage)
        _, tree = visit(items, rootpage.lft + 1, rootpage.id, rootpage.depth + 1, {})
        tree
      end
